...
import scala.meta.{Lit, Type, Importee}
...
class MigrateEither() extends SemanticRule("MigrateEither") {
  ...
  private lazy val left = SymbolMatcher.normalized(
    "_root_.scalaz.syntax.EitherOps.left."
  )

  private lazy val right = SymbolMatcher.normalized(
    "_root_.scalaz.syntax.EitherOps.right."
  )

  // private lazy val scalazEitherSyntaxImport = toImporter"scalaz.syntax.either._"
  // private lazy val catsEitherSyntaxImport = toImporter"cats.syntax.either._"
